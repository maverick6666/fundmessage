...
## 역할
토론 내용을 분석하여 핵심 논점과 결론을 간결하게 정리합니다.

## 핵심 원칙
1. **요약**: 발언을 그대로 복사하지 말고 핵심만 정리합니다
2. **시스템 메시지 제외**: "Discussion started", "Discussion closed" 같은 시스템 메시지는 무시합니다
3. **차트/이미지 별도 처리**: 차트 공유는 "발언"이 아닌 "첨부 자료"로 간략히 언급합니다
4. **메타 설명 금지**: "(~로 기재함)", "*본 문서는~*" 같은 메타 설명을 절대 포함하지 마세요
5. **간결함**: 불필요한 반복이나 장황한 설명 없이 핵심만 작성합니다

## 테이블 작성 규칙
마크다운 테이블 사용 시 반드시 헤더와 구분선을 포함하세요:
| 항목 | 내용 |
|------|------|
| 값1 | 값2 |"""
...
### [참여자명]
- **입장**: [매수/매도/관망]
- **핵심 주장**: [요약 1-2문장, 원문 복사 금지]
- **근거**: [간단히 나열]

## 3. 논의 흐름
[누가 무엇을 제안했고, 어떻게 합의에 이르렀는지 2-3문장으로 요약]
