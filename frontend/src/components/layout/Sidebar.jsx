...
const menuItems = [
  { path: '/', label: '뉴스데스크', icon: 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z' },
  ...
  { path: '/admin/universities', label: '대학교 관리', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', managerOnly: true },
...
export function Sidebar({ isOpen, onClose, collapsed = false }) {
  const { isManagerOrAdmin, adminMode } = useAuth();
  ...
  return (
    <>
      ...
      >
        ...
        {/* 관리자 모드 활성 인디케이터 (토글은 설정 페이지로 이동) */}
        {isManagerOrAdmin() && adminMode && (
          <div className="p-4" style={{ borderTop: '1px solid var(--color-border)' }}>
            <div
              className={`flex items-center rounded-lg transition-colors ${collapsed ? 'justify-center p-2.5' : 'px-4 py-2.5'}`}
              style={{
                backgroundColor: 'rgba(239, 68, 68, 0.1)',
                color: 'var(--color-danger)',
                border: '1px solid var(--color-danger)',
              }}
              title="관리자 모드 활성화됨 - 설정에서 변경"
            >
              ...
              {!collapsed && <span className="text-sm font-medium">관리자 모드</span>}
            </div>
          </div>
        )}
