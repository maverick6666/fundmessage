...
|--------|------|---------------|---------------|------|
| 1 | 2026-02-07 | 3개 버그 + 2개 UX개선 | 0 | 문제 해결 단계 |
| 2 | | | | 대기 |
| 3 | | | | 대기 |
...
|---|----------|----------|-----------|-----------|
| 1 | 전역 | **alert() 대화상자가 toast 대신 사용됨** - 배포된 버전에 toast 변환이 적용되지 않음. 로컬 코드는 변환 완료되었으나 git push 후 재배포 필요 | frontend/src/pages/*.jsx | 미해결 - git push 필요 |
...
| # | 위치 | 개선 내용 | 파일/위치 | 상태 |
|---|------|----------|-----------|------|
| 1 | 6.2 포지션 상세 | **차트 섹션 높이 고정** - 매매계획이 길어져도 차트 섹션이 같이 늘어나지 않도록 고정. `items-start` + `lg:self-start` 추가 | frontend/src/pages/PositionDetail.jsx | 수정됨 - 테스트 필요 |
| 2 | 4.1 요청 목록 | **수량 소수점 과다 표시** - `formatQuantity()` 함수 사용하여 불필요한 소수점 제거 | frontend/src/pages/Requests.jsx | 수정됨 - 테스트 필요 |
| 3 | 3.1 종목 검색 | **차트 인라인 표시** - 종목 선택 후 차트 보기 시 팝업 대신 검색 컴포넌트 내부에 표시 | frontend/src/pages/Positions.jsx | 수정됨 - 테스트 필요 |
