...
## N. 복합 플로우 시나리오 (Integration Tests)
...
| 단계 | 수행자 | 액션 | 예상 결과 |
|------|--------|------|----------|
| 1 | 팀원 | 포지션 요청 → 매수 요청 제출 | 요청 생성 + 팀장에게 알림 |
| 2 | 팀장 | 요청 목록 확인 | 대기중 요청 표시 |
| 3 | 팀장 | "토론" 버튼 클릭 → 제목 입력 | 토론방 생성 + 요청 상태 "토론중" |
| 4 | 팀원 | 알림 수신 확인 | 토론 개시 알림 |
| 5 | 팀원/팀장 | 토론방에서 의견 교환 | 실시간 메시지 |
| 6 | 팀장 | "토론 종료" 클릭 | 토론 종료됨 |
| 7 | 팀장 | 요청 목록 → 승인 클릭 | 포지션 생성 (미확인 상태) |
| 8 | 팀원 | 알림 수신 확인 | 요청 승인 알림 |

### N2. 열린 토론 경고 시나리오 ⚠️
**사전 조건:**
- 대기중인 요청에 열린 토론이 있는 상태

| 단계 | 수행자 | 액션 | 예상 결과 |
|------|--------|------|----------|
| 1 | 팀장 | 요청에서 "토론" 클릭 | 토론방 생성 |
| 2 | 팀장 | 요청 목록으로 복귀 | 토론 열린 상태 |
| 3 | 팀장 | "승인" 클릭 | ⚠️ 경고 모달: "열린 토론이 있습니다" |
| 4a | - | "토론 종료 후 승인" 선택 | 토론 자동 종료 + 승인 진행 |
| 4b | - | "그냥 승인" 선택 | 토론 열린 채 승인 |
| 4c | - | "취소" 선택 | 아무 동작 안함 |
| 5 | 팀장 | "거부" 클릭 (토론 열린 상태) | ⚠️ 동일한 경고 모달 |

**주의:** 이 기능은 미구현 상태 - Task #5에서 구현 필요
...
| 단계 | 수행자 | 액션 | 예상 결과 |
|------|--------|------|----------|
| 1 | 팀장 | 포지션 상세 페이지 접속 | 노란 느낌표 배지 (미확인) |
| 2 | 팀장 | 편집 버튼 클릭 | 수정 모드 활성화 |
| 3 | 팀장 | 평균 매입가 입력 | 입력값에 불필요한 0 없음 (예: 56200, 56200.5) |
| 4 | 팀장 | 수량 입력 | 동일 |
| 5 | 팀장 | 저장 클릭 | 성공 토스트 + 배지 제거 + 수정 이력 기록 |

### N4. 매매계획 수정 및 이력 시나리오
**핵심 검증 포인트:**
- 체크박스 클릭 시 이력 생성 ❌ (생성되면 안됨)
- "현재 계획 저장" 클릭 시에만 이력 생성 ✅

| 단계 | 수행자 | 액션 | 예상 결과 |
|------|--------|------|----------|
| 1 | 팀원/팀장 | 익절가 추가 → 가격/수량 입력 → ✓ 클릭 | 항목 추가됨, **이력 미생성** |
| 2 | - | 손절가 추가 → 가격/수량 입력 → ✓ 클릭 | 항목 추가됨, **이력 미생성** |
| 3 | - | 기존 항목 수정 → ✓ 클릭 | 수정됨, **이력 미생성** |
| 4 | - | "이력 보기" 클릭 | 이력 목록 (아직 비어있거나 이전 것만) |
| 5 | - | "현재 계획 저장" 클릭 | ✅ **이력 1개 생성** + 성공 토스트 |
| 6 | - | "이력 보기" 클릭 | 방금 저장한 스냅샷 표시 |
| 7 | - | 다시 수정 → "현재 계획 저장" | ✅ **이력 1개 추가** (총 2개) |

**주의:** 현재 구현 확인 필요 - Task #6

### N5. 매매계획 체결 실행 시나리오
**사전 조건:**
- 익절/손절 계획이 설정된 진행중 포지션

| 단계 | 수행자 | 액션 | 예상 결과 |
|------|--------|------|----------|
| 1 | 팀장 | 익절가 항목 체크박스 클릭 | ⚠️ 체결 확인 모달 |
| 2 | - | 모달에서 실제 체결가/수량 확인/수정 | 입력 가능 |
| 3 | - | "체결 완료" 클릭 | 항목 "완료" 상태 + ✅ **이력에 기록** |
| 4 | - | "이력 보기" 클릭 | "익절 체결: ₩XX,XXX × X주 완료" 형식 표시 |
| 5 | - | 손절가 체결 | 동일하게 이력 기록 |
| 6 | - | 대시보드/통계 확인 | 실현손익에 반영됨 |
| 7 | - | 포지션 거래대금 확인 | 체결 금액 반영됨 |

**주의:** 체결 이력과 실현손익 연동 - Task #6에서 구현 필요
...
| 단계 | 수행자 | 액션 | 예상 결과 |
|------|--------|------|----------|
| 1 | 팀장 | 포지션 상세 → "토론방 열기" | 제목 입력 모달 |
| 2 | 팀장 | "매매계획 논의" 제목으로 생성 | 토론방 생성 |
| 3 | 팀원 | 토론방 접속 → 의견 제시 | 메시지 전송 |
| 4 | 팀장 | 차트 공유 → 목표가 논의 | 차트 메시지 |
| 5 | 팀원 | 합의된 내용으로 익절가 수정 | 계획 업데이트 |
| 6 | 팀장 | 토론 종료 | 토론 종료됨 |
| 7 | 팀원/팀장 | "현재 계획 저장" | 합의된 계획 이력 저장 |

### N7. 운용보고서 작성 시나리오
**핵심 검증:**
- 포지션 종료/진행중 관계없이 보고서 작성 가능

| 단계 | 수행자 | 액션 | 예상 결과 |
|------|--------|------|----------|
| 1 | 팀장 | 진행중 포지션 상세 접속 | 보고서 작성 버튼 표시 |
| 2 | 팀장 | "AI 보고서" 클릭 | AI 생성 모달 |
| 3 | 팀장 | 생성 및 저장 | 의사결정노트에 추가 |
| 4 | 팀장 | 종료된 포지션 상세 접속 | 동일하게 버튼 표시 |
| 5 | 팀장 | "AI 보고서" 클릭 | AI 생성 가능 |
...
|------|------|------|
| 요청 승인/거부 버튼 | ✅ 표시 | ❌ 숨김 |
| 토론방 열기 버튼 | ✅ "토론방 열기" | ✅ "토론 요청" |
| 토론 재개 버튼 | ✅ "토론 재개" | ✅ "재개 요청" |
| 포지션 종료 버튼 | ✅ "포지션 종료" | ✅ "조기종료 요청" |
| 포지션 정보 수정 | ✅ 편집 버튼 | ❌ 숨김 |
| 매매계획 체결 체크박스 | ✅ 표시 | ❌ 숨김 |
| 팀 관리 메뉴 | ✅ 표시 | ❌ 숨김 |
| 관리자 모드 | ✅ 토글 가능 | ❌ 숨김 |
...
| # | 시나리오 | 문제 | 파일 | 상태 |
|---|----------|------|------|------|
| 1 | E5. 토론 재개 요청 | 버튼 중복 클릭 시 알림 중복 생성 | Discussion.jsx | ✅ 해결 |
| 2 | G1. 팀 전체 통계 | 차트 데이터가 더미(랜덤)로 8월부터 표시 | Stats.jsx | 🔍 원인파악 |
| 3 | C4. 포지션 정보 수정 | 입력창에 불필요한 0 표시 (56200.0000) | PositionDetail.jsx | ✅ 해결 |

## 미구현 기능 (구현 필요)

| # | 시나리오 | 기능 | 우선순위 |
|---|----------|------|----------|
| 1 | N2. 열린 토론 경고 | 승인/거부 시 열린 토론 있으면 경고 모달 | 높음 |
| 2 | N4. 매매계획 이력 | 체크박스가 아닌 "현재 계획 저장"만 이력 생성 | 중간 |
| 3 | N5. 매매계획 체결 | 체결 시 이력 + 실현손익/거래대금 연동 | 높음 |
| 4 | B1. 대시보드 | 달러 금액에 원화 환산 표시 (≈₩) | 낮음 |
| 5 | C5. 매매계획 | 포지션 그라데이션 70%+ 조건 | 낮음 |
| 6 | G1. 통계 차트 | 실제 포트폴리오 이력 기반 차트 데이터 | 중간 |
...
| 단계 | 액션 | 예상 결과 |
|------|------|----------|
| 1 | `/newsdesk` 접속 | 뉴스데스크 페이지 로드 |
| 2 | - | 오늘 날짜 뉴스데스크 표시 (있는 경우) |
| 3 | - | 벤치마크 차트 표시 |
| 4 | - | AI 칼럼, 뉴스 카드, 키워드 클라우드, 감성 게이지, 주목 종목 |

### O2. 날짜 선택기
| 단계 | 액션 | 예상 결과 |
|------|------|----------|
| 1 | 날짜 드롭다운 클릭 | 최근 7일 히스토리 표시 |
| 2 | 이전 날짜 선택 | 해당 날짜 뉴스데스크 로드 |
| 3 | < 버튼 클릭 | 하루 전으로 이동 |
| 4 | > 버튼 클릭 | 하루 후로 이동 (오늘까지만 가능) |

### O3. 벤치마크 차트
| 단계 | 액션 | 예상 결과 |
|------|------|----------|
| 1 | 벤치마크 토글 클릭 (코스피, 나스닥, S&P500, 우리팀) | 차트에 시리즈 추가/제거 |
| 2 | 1개만 선택 | 가격 표시 (원래 값) |
| 3 | 2개 이상 선택 | 수익률로 정규화 (첫날=0%, 변화율) |
| 4 | 기간 선택 (1W, 1M, 3M, 6M, 1Y) | 차트 데이터 기간 변경 |
...
| 단계 | 액션 | 예상 결과 |
|------|------|----------|
| 1 | "지금 생성하기" 또는 "새로고침" 클릭 | 생성 중 로딩 표시 |
| 2 | - | AI가 뉴스 수집 및 분석 (1-2분) |
| 3 | 생성 완료 | 성공 토스트 + 뉴스데스크 표시 |
...
| 상태 | 표시 내용 |
|------|----------|
| pending | "뉴스데스크 준비 중..." + 생성하기 버튼 (팀장) |
| generating | AI 분석 중 애니메이션 |
| failed | 에러 메시지 + 다시 시도 버튼 + "이전 뉴스데스크 보기" |
| empty | "선택한 날짜의 뉴스데스크가 없습니다" + 이전 기록 보기 |

### O6. 뉴스/칼럼 상세 보기
| 단계 | 액션 | 예상 결과 |
|------|------|----------|
| 1 | AI 칼럼 카드 클릭 | 사이드 패널에 상세 표시 |
| 2 | 뉴스 카드 클릭 | 사이드 패널에 상세 표시 |
| 3 | 주목 종목 카드 클릭 | 사이드 패널에 상세 표시 |
| 4 | 원문 보기 링크 클릭 | 새 탭에서 원문 열림 |

### O7. 키워드 클라우드
| 단계 | 액션 | 예상 결과 |
|------|------|----------|
| 1 | 키워드 버블 클릭 | 선택 상태 (ring 표시) |
| 2 | 다시 클릭 | 선택 해제 |
| 3 | - | 감성 게이지에 선택된 키워드 표시 |
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-02-08 | O. 뉴스데스크 시나리오 7개 추가 (O1~O7) |
| 2026-02-08 | N. 복합 플로우 시나리오 8개 추가 (N1~N8) |
| 2026-02-08 | 미구현 기능 목록 추가 |
