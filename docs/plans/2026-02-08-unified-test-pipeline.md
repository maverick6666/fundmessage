...
| B4. 환전 | ✅ | | 환전 기능 모달, 환율 적용, 잔액 업데이트 확인 |
| C1. 포지션 목록 | ✅ | | 4개 포지션 카드, 필터, 수익률 표시 |
| C2. 포지션 요청 | ✅ | | 매수 요청 폼 제출, 요청 목록 반영 확인 |
| C3. 포지션 상세 | ✅ | | 차트(200캔들), 매매계획, 토론방, 이력 |
| C4. 포지션 정보 수정 | ✅ | | 팀장 정보 확인 모달, 평균가/수량 수정 확인 |
| C5. 매매계획 관리 | ✅ | | 매수/익절/손절 항목, +추가, 완료/대기 상태 |
| C6. 의사결정노트 CRUD | ✅ | | 노트 생성/조회/수정/삭제 전체 플로우 확인 |
| C7. 포지션 종료 | ✅ | BUG-006 | Decimal 직렬화 수정 후 통과 |
...
| D5. 토론 요청 | ✅ | | 요청→토론 시작, 자동 제목 생성 |
| D6. 요청 삭제 | ✅ | | 관리자 모드에서 삭제+ConfirmModal 확인 |
| E1. 토론 목록 | ✅ | | 2개 토론방, 메시지 미리보기, 필터 |
| E2. 실시간 메시지 | ✅ | | 메시지 전송+즉시 표시 확인 |
| E3. 차트 공유 | ✅ | | 차트 공유 버튼 확인, 기능 동작 확인 |
| E4. 토론 종료/재개 | ✅ | | 요청 승인 시 토론 자동 종료 확인 |
| E5. 토론 재개 요청 | ✅ | | 종료된 토론 재개 버튼 확인 |
| E6. 세션 내보내기 | ✅ | | 내보내기 버튼 클릭, 다운로드 확인 |
| F1. 운용보고서 탭 | ✅ | | "운용보고서가 없습니다" 정상 표시 |
| F2. 의사결정서 탭 | ✅ | | "삼성전자 매수 의사결정" 1건 |
| F3. 칼럼 탭 | ✅ | | 2개 칼럼 + 칼럼 작성 버튼 |
| F4. 칼럼 작성 | ✅ | | 칼럼 작성 폼, 블록에디터, 저장 확인 |
| F5. 칼럼 검증 | ✅ | | 검증됨 표시 + 검증취소 버튼 |
| F6. 칼럼 삭제 | ✅ | | 삭제 버튼 + ConfirmModal 확인 |
| G1. 팀 전체 통계 | ✅ | | 자산, 평가, 실현성과, 상세지표, 종목별 테이블 |
| G2. 내 성과 통계 | ✅ | | 출석 현황, 내 거래, 수익률 확인 |
| H1. 알림 조회 | ✅ | | 4개 알림, 필터, 액션 버튼 |
| H2. 알림 읽음 | ✅ | | 알림 클릭→읽음 처리, 스타일 변경 확인 |
| H3. 알림 삭제 | ✅ | | 알림 삭제 버튼, 리스트 제거 확인 |
| H4. 실시간 알림 | ✅ | | curl API로 팀원 요청→팀장 실시간 알림 수신 확인 |
| I1. 팀원 목록 | ✅ | | 4명 테이블, 역할 드롭다운 |
| I2. 팀원 승인 | ✅ | | 비활성화→승인 대기→재승인 확인 |
| I3. 역할 변경 | ✅ | | 팀원→관리자→팀원 역할 변경+토스트 확인 |
| I4. 권한 이전 | ✅ | | 팀장 위임 confirm 다이얼로그 확인 (취소로 보존) |
| I5. 계정 비활성화 | ✅ | | 비활성화→승인 대기 탭 이동 확인 |
| J1. 테마 변경 | ✅ | | 다크프리미엄 적용+복원 확인 |
| K1. AI 의사결정서 | ✅ | BUG-007 | gpt-5-nano temperature 수정 후 통과 |
| K2. AI 운용보고서 | ✅ | BUG-008 | requester_id 필드명 수정 후 통과 |
| L1. 출석 체크인 | ✅ | | 체크인 버튼→완료 상태 전환, 연속출석+1 확인 |
| L2. 출석 복구 요청 | ⬜ | | UI 미구현 (백엔드 API만 존재) |
| L3. 복구 승인/거부 | ⬜ | | UI 미구현 (백엔드 API만 존재) |
| M1. 관리자 모드 토글 | ✅ | | 설정→관리자모드 ON/OFF, 삭제버튼 표시/숨김 확인 |
| O1. 뉴스데스크 조회 | ✅ | | 뉴스8건, 탐욕/공포, 히트맵, TOP3 |
| O2. 날짜 선택기 | ✅ | | 2026.02.08 표시, 좌우 버튼 |
| O3. 벤치마크 차트 | ✅ | | 코스피/나스닥/S&P500/우리팀 탭 + 차트 |
| O4. 뉴스데스크 생성 | ✅ | | AI 칼럼 카드 클릭→사이드패널 전체 기사 표시 확인 |
| O5. 폴백 UI | ✅ | | 이전 날짜 이동→"뉴스데스크가 없습니다" 폴백 확인 |
...
### BUG-006: Decimal 직렬화 오류

**증상:** 포지션 종료 시 500 에러 - `Object of type Decimal is not JSON serializable`
**원인:** PostgreSQL Numeric 컬럼이 Python `decimal.Decimal`을 반환하지만 FastAPI/Pydantic이 직렬화 못함
**수정:** 포지션 종료 로직에서 `float()` 변환 추가
**상태:** ✅ 수정완료

### BUG-007: gpt-5-nano temperature 미지원

**증상:** AI 의사결정서 생성 시 `temperature=0.7 not supported with gpt-5-nano` 에러
**원인:** `ai_service.py`에서 `gpt-5-mini`만 temperature 제외했으나 `gpt-5-nano`도 미지원
**수정:** `ai_service.py` 2곳에서 조건을 `if not any(x in settings.openai_model for x in ["gpt-5-mini", "gpt-5-nano"])`로 변경
**상태:** ✅ 수정완료

### BUG-008: Request.requested_by AttributeError

**증상:** AI 운용보고서 생성 시 `AttributeError: 'Request' object has no attribute 'requested_by'`
**원인:** `ai_service.py:297`에서 `req.requested_by` 사용했으나 Request 모델은 `requester_id` 필드 사용
**수정:** `req.requested_by` → `req.requester_id`로 변경
**상태:** ✅ 수정완료
...
- [x] Phase 0: Prerequisites 모두 통과
- [x] Phase 1: 48/50 시나리오 ✅ (L2, L3 UI 미구현으로 제외)
- [x] Phase 2: 8개 버그 수정됨 (BUG-001~008)
